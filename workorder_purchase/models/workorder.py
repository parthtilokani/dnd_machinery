...
class MRPWorkOrder(models.Model):
    ...
    def action_subcontract(self):
        ...
        process_id = bom_id.process_ids.filtered(lambda x: x.process_id.mrp_workcenter_id == self.workcenter_id)
...
class MRPProductionInherit(models.Model):
    ...
    resupply_order_count = fields.Integer(string="Ticket Count", compute="compute_custom_resupply_order", copy=False)

    def compute_custom_resupply_order(self):
        for record in self:
            resupply_orders = self.env['stock.picking'].search([('purchase_id.manufecturing_id', '=', record.id)])
            record.resupply_order_count = len(resupply_orders)

    def action_view_resupply_order(self):
        resupply_orders = self.env['stock.picking'].search([('purchase_id.manufecturing_id', '=', self.id)])
        if resupply_orders:
            return {
...
class PurchaseOrderInherit(models.Model):
    ...
    manufecturing_id = fields.Many2one('mrp.production', string="Manufacturing No")
